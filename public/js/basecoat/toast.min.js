;(() => {
	...
	function i(e) {
		e.dataset.toasterInitialized ||
			...
			t.addEventListener('click', (t) => {
				...
				;(e || n) && d(t.target.closest('.toast'))
			}),
			t.querySelectorAll('.toast:not([data-toast-initialized])').forEach(a),
	...
	function a(t) {
		...
		;(-1 !== i &&
			(n ?
				(a.timeoutId = null)
			:	((a.startTime = Date.now()), (a.timeoutId = setTimeout(() => d(t), i)))),
			e.set(t, a),
		...
		n &&
			...
			t.querySelectorAll('.toast:not([aria-hidden="true"])').forEach((t) => {
				...
				;-1 === n.remainingTime ||
					n.timeoutId ||
					(n.remainingTime > 0 ?
						((n.startTime = Date.now()), (n.timeoutId = setTimeout(() => d(t), n.remainingTime)))
					:	d(t))
			}))
	}
	function d(t) {
	...
	;(c && i(c),
		document.addEventListener('basecoat:toast', (e) => {
			if (!t) return void console.error('Cannot create toast: toaster container not found on page.')
			const n = (function (t) {
				const {
						category: e = 'info',
						title: n,
					...
					u = i ? `<p>${i}</p>` : '',
					h =
						a?.href ? `<a href="${a.href}" class="btn" data-toast-action>${a.label}</a>`
						: a?.onclick ?
				...
				return ((v.innerHTML = g.trim()), v.content.firstChild)
			})(e.detail?.config || {})
			t.appendChild(n)
		}))
		...
		e.forEach((e) => {
			e.addedNodes.forEach((e) => {
				e.nodeType === Node.ELEMENT_NODE &&
					(e.matches('#toaster') && i(e),
					t && e.matches('.toast:not([data-toast-initialized])') && a(e))
			})
		})
