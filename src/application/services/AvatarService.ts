...
import { InvalidParamError, AvatarNotFoundError } from '../../domain/exceptions.ts';
...
export class AvatarService {
  ...
  async generate(dto: AvatarProps): Promise<{ data: Uint8Array; type: string }> {
    ...
    try {
      ...
      if (dto.text) {
        const svg = `<svg width="${avatar.size.value}" height="${avatar.size.value}" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" fill="${avatar.bg.value}"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-size="${avatar.size.value / 2}" fill="${avatar.fg.value}">${dto.text}</text></svg>`;
    ...
    const buffer = await processAvatar(data, avatar.size.value, avatar.format.value, avatar.shape.value);
