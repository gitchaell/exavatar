...
export async function processAvatar(data: Uint8Array, size: number, format: ImageFormat, shape: AvatarShape) {
  ...
  if (shape === 'circle') {
    ...
    const svg = `<svg><rect x="0" y="0" width="${size}" height="${size}" rx="${size * 0.25}" ry="${size * 0.25}"/></svg>`;
    img = img.composite([{ input: Buffer.from(svg), blend: 'dest-in' }]);
  }
